...
#define NEXT_BLKP(bp) \
  ((char *)(bp) + GET_SIZE(((char *)(bp)-WSIZE)))  // 다음 블럭 포인터
#define PREV_BLKP(bp) \
  ((char *)(bp)-GET_SIZE(((char *)(bp)-DSIZE)))  // 이전 블럭 포인터
...
int mm_init(void) {
  ...
  PUT(heap_listp + (1 * WSIZE),
      PACK(DSIZE, 1));  // 프롤로그 헤더 (자기 블럭의 전체 용량을 말함 ~PACK)
  PUT(heap_listp + (2 * WSIZE),
      PACK(DSIZE, 1));  // 프롤로그 풋터 (자기 블럭의 전체 용량을 말함 ~PACK)
...
static void *coalesce(void *bp) {
  size_t prev_alloc =
      GET_ALLOC(FTRP(PREV_BLKP(bp)));  // 이전 블록의 할당 상태 조사
  size_t next_alloc =
      GET_ALLOC(HDRP(NEXT_BLKP(bp)));  // 다음 블록의 할당 상태 조사
  size_t size =
      GET_SIZE(HDRP(bp));  // 매개변수로 받은 bp 헤더포인트의 블록 사이즈 조사
  ...
  if (prev_alloc && next_alloc) {  // Case 1 : prev next 모두 할당되어 있음
    ...
    size += GET_SIZE(
        HDRP(NEXT_BLKP(bp)));  // 현재 블록의 사이즈와 다음 블록 사이즈 더하기
    ...
    PUT(FTRP(NEXT_BLKP(bp)),
        PACK(size, 0));  // 이쪽은 아직 bp를 못받았기 때문에 내 위치 기준으로
                         // 풋터를 찾아가야 함
    bp = PREV_BLKP(bp);
  }
...
void *mm_malloc(size_t size) {
  ...
  } else {  // 할당 요청 사이즈가 더블워드 이상 (size / DSIZE) * DSIZE +
            // DSIZE(여분) + DSIZE(헤더풋터)
...
void mm_free(void *bp) {  //매개변수명 ptr에서 bp로 수정함
  ...
  PUT(HDRP(bp),
      PACK(size, 0));  // 헤더에 PACK을 반영, 이 블럭을 free 상태로 수정
  PUT(FTRP(bp),
      PACK(size, 0));  // 풋터에 PACK을 반영, 이 블럭을 free 상태로 수정

  //*******재확인
  coalesce(bp);  // 인접한 가용 영역과 통합 ~근데 이거 매번 하는거 아니라고 하지
                 // 않았나..
}
